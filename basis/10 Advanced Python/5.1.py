...
Сейчас значения в каждом списке перемешаны и не понятно какому работнику
какой идентификатор принадлежит. Отдел кадров просит вас создать словарь employees_data, в котором, 
ключами будут идентификаторы, а значениями - имена работников. При этом отдел кадров просит соотнести 
идентификатор работника с именем следующим образом:
...
result = dict(zip(sorted(identifiers), sorted(employees)))
print(result)